...
int main(int argc, char *argv[]) {
	...
	//Keep track of how long the file is and what word we are at
	fileCounter = 0;
	...
	for (i1 = 0; i1 < dictSize; i1++) {
		dictionary[vec[i1].first] = i1;
		dictionaryVec.push_back(vec[i1].first);
	}
	...
	//Collect X on the root
	if (rank == 0) {
		//Go through each row and receive it from the correct node that processed it
		for (i1 = 1; i1 < r; i1++) {
			if (i1 % size == 0) {
				continue;
			}
	...
	if (rank == ROOT) {
		...
		//Go through each row and receive it from the node that processed it
		for (i1 = 1; i1 < c; i1++) {
			if (i1 % size == 0) {
				continue;
			}
